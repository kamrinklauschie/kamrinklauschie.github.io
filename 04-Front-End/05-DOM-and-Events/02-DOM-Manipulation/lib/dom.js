...
module.exports = function runChallenges(check) {
  ...
  const email = document.querySelector('#email');
  email.value = "kamrin.klauschie@gmail.com";
  ...
  element.insertAdjacentHTML('beforeend', "<tr><td>15</td><td>Paris Saint Germaine</td><td>0</td></tr>");
  ...
  const summarizePoints = () => {
    // TODO: return the sum
    let sum = 0;
    const elements = document.querySelectorAll('tbody tr td:nth-child(3)');
  ...
  cells.forEach((cell) => {
    cell.style.backgroundColor = "rgb(221, 244, 255)";
  });
